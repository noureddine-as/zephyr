...
void riscv_plic_irq_enable(u32_t irq)
{
	...
	volatile u32_t *en =
		(volatile u32_t *)DT_INST_0_LOWRISC_PLIC_1_0_0_IRQ_EN_BASE_ADDRESS;
...
{
	volatile u32_t *prio =
		(volatile u32_t *)DT_INST_0_LOWRISC_PLIC_1_0_0_PRIO_BASE_ADDRESS;

	/* Can set priority only for PLIC-specific interrupt line */
	if (irq <= RISCV_MAX_GENERIC_IRQ)
		return;

	if (priority > DT_INST_0_LOWRISC_PLIC_1_0_0_RISCV_MAX_PRIORITY)
		priority = DT_INST_0_LOWRISC_PLIC_1_0_0_RISCV_MAX_PRIORITY;
...
static void plic_irq_handler(void *arg)
{
	volatile struct plic_regs_t *regs =
	    (volatile struct plic_regs_t *) DT_INST_0_LOWRISC_PLIC_1_0_0_REG_BASE_ADDRESS;
